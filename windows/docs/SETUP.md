...
Windows PowerShell 使用 `npm.cmd`；系统和桌面构建要求见 [Windows 安装说明](../README-WINDOWS.md)。Mac 要求 Node.js 20.19+；原生桌面另需 Swift/Xcode Command Line Tools。保留设置窗口，打开它显示的本机链接。链接含会话授权，不要截图公开或转发。首次入口只启动本地设置服务，不会打开设备、登录账户或自动调用模型。已有配置时使用原桌宠入口，在控制台“模型与声音”中修改。
...
Windows 使用 `npm.cmd run build:windows`，具体依 [Windows 安装说明](../README-WINDOWS.md)。资源齐备后，在设置页点击完成，只生成“已准备”配置。接着显式启用：
...
Mac Harness 使用本机服务与预设连接，当前适配协议为 0.1.5-rc.1。Mac Codex 连接需要受支持的桌面 App 构建和本机状态，安装后请检查连接状态。Windows 保留现有官方 `codex app-server --stdio` 适配和 Harness 单独配置流程，具体版本、目录和登录步骤以[平台说明](../README-WINDOWS.md)为准。
