...
function assertOwned(path:string,directory=false):void {
  ...
  if(stat.isSymbolicLink() || (directory?!stat.isDirectory():!stat.isFile()) || (stat.mode&0o077)!==0 || process.getuid&&stat.uid!==process.getuid())
    fail('unavailable','本机凭据存储权限不符合要求。');
}
function readOwned(path:string):string {
  assertOwned(path);
  const fd=openSync(path,constants.O_RDONLY|constants.O_NOFOLLOW);
  try {const opened=fstatSync(fd),current=lstatSync(path);if(opened.dev!==current.dev||opened.ino!==current.ino||opened.size>256*1024)fail('unavailable','本机凭据存储状态不可用。');return readFileSync(fd,'utf8');}
...
export function acquireSetupLock(file:string,scope:string):()=>void {
  ...
  try{writeFileSync(fd,JSON.stringify(identity)+'\n');}catch(error){closeSync(fd);throw error;}
  const opened=fstatSync(fd);let released=false;
  return()=>{if(released)return;released=true;closeSync(fd);const current=lstatSync(file);if(current.ino!==opened.ino||current.dev!==opened.dev)throw Error('setup_lock_changed');unlinkSync(file);};
}
...
export class ManagedCredentialStore {
  ...
  private ensureDirectory():void {
    ...
    for(const next of missing.reverse())mkdirSync(next,{mode:0o700});
    assertOwned(this.directory,true);
  }
  save(input:{provider:ManagedCredentialProvider;key:string;expectedRevision:number;operationId:string}):{revision:number;credentialRef:string;provider:ManagedCredentialProvider} {
    ...
    try {
      ...
      writeFileSync(file,key!,{flag:'wx',mode:0o600});created=file;assertOwned(file);
      ...
      temporary=join(this.directory,'registry-'+randomUUID()+'.next');writeFileSync(temporary,JSON.stringify(next)+'\n',{flag:'wx',mode:0o600});renameSync(temporary,join(this.directory,'registry.json'));temporary=undefined;created=undefined;
